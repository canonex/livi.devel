...
def register():
    ...
    Scene.livi_export_time_type = EnumProperty(
            ...
            name="", description="Specify the time type", default="0")      
    ...
    Scene.livi_export_summer_meridian = EnumProperty(
            items=[("0", "YDT", ""),
                   ...
                   ("11", "JDT", ""),
                   ("12", "NZDT", ""),                   ],
            name="Meridian", description="Specify the local Summertime meridian", default="BST")
...
# LiVi Calculation panel ui elements
    ...
    Scene.livi_calc_acc = EnumProperty(
            ...
            name="", description="Specify the speed and accuracy of the simulation", default="0")
    
    Scene.livi_calc_dastart_hour = IntProperty(
            name="Hour", description="Starting hour for occupancy", min=1, max=24, default=8) 
            
    Scene.livi_calc_daend_hour = IntProperty(
            name="Hour", description="Ending hour for occupancy", min=1, max=24, default=19)
            
    Scene.livi_calc_min_lux = IntProperty(
            name="Lux", description="Minimum Lux level required", min=1, max=2000, default=200)
            
    Scene.livi_calc_da_weekdays = BoolProperty(
            name="Weekdays only", description="Calculate Daylight availability for weekdays only", default=True)
...
# LiVi Display panel properties
    Scene.livi_display_legend = IntProperty(
            name="Legend Display", description="Shows a colour coded legend for the results in the 3D view", default=0)
    ...
    Scene.livi_disp_3d = BoolProperty(
            name="3D Display", description="Enable 3D results analysis", default=0)
            
    Scene.livi_render_view = BoolProperty(
            name="OpenGL Display", description="Enable OpenGL 3D results view", default=True) 
            
    Scene.livi_disp_3dlevel = FloatProperty(
            name="3D level", description="Level of 3D effect", min=0.1, max=5000, default=2)
          
    Scene.livi_display_respoints = BoolProperty(
            name="Display", description="Display results on vertices/faces", default=False)

    Scene.livi_display_sel_only = BoolProperty(
            name="Selected only", description="Only display results on selected objects", default=True)
    
    Scene.livi_display_rp_fs = IntProperty(
            name="Font size", description="Font size for point display", default=13)
